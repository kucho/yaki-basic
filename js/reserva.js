...
function agregar_mesa() {
	...
	while (cap < 0 || cap > 12 || isNaN(cap) == true) {
		...
		if (cap < 0 || cap > 12 || isNaN(cap) == true) {
			if (!confirm("El número ingresado no es válido. Cada mesa puede tener una capacidad máxima de 12 comensales. ¿Desea volver a intentarlo?")) {
				return false
			}
		}
	}
	...
	if (mesas.length > 0) {
		...
		let mesa_id = mesas[mesas.length - 1].parentNode.getAttribute(
			"id"
		)
...
function quitar_mesa() {
	...
	if (selec.length > 0) {
		...
		for (let i = 0; i < selec.length; i++) {
			...
			if (confirm(`¿Está seguro de querer eliminar la mesa ${id_mesa}? Se perderán todos los registros de la agenda.`)) {
				/* Eliminamos la mesa seleccionada */
				selec[i].parentElement.removeChild(selec[i])

				/* Eliminamos la agenda vinculada */
				let agenda = document.getElementById(
					`agenda-${id_mesa}`
				)
...
function guardar_reserva() {
	...
	if (
		...
		alert("La cantidad de comensales excede la capacidad de la mesa. Por favor seleccione una nueva mesa o agregue más capacidad.")
		return false
	}
...
function crear_cod(tamaño) {
	...
	for (var i = 0; i < tamaño; i++) {
		resultado += caracteres.charAt(
			Math.floor(Math.random() * carac_largo)
		)
	}
	return resultado
}
